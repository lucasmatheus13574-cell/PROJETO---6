...
Backend: Node.js, Express.js, SQLite, bcryptjs, jsonwebtoken.


4. Funcionalidades Implementadas (Missão 6):
Registro de novos usuários.
Login de usuários existentes.
Proteção de rotas de tarefas por autenticação (JWT).
Visualização, adição, edição (marcar/desmarcar), e exclusão de tarefas apenas para usuários logados.
Persistência de dados de usuários e tarefas no SQLite.
...
7. "Credenciais de Teste": Inclua um username e password de um usuário de teste que eu possa usar para fazer login e testar as funcionalidades protegidas da API. Se for preciso registrar esse usuário primeiro, por favor, informar no arquivo.
